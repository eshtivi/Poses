...
class MyCenter: osg::NodeVisitor
{
public:
	osg::Vec3f center;

	MyCenter(osg::Vec3f c)
	{
		center = c;
	}
};
...
osg::Matrix calcLookAt(osg::ref_ptr<osg::Node> node) {
...
osg::Matrix getModelMatrix(osg::Node *node)
{
	osg::Matrix mm;
...
osg::Matrix CalcTransformation(double axisX, double axisY, double axisZ, double azimuth, double elevation, double yaw, string deg_order)
{
	osg::Matrix trans = osg::Matrix::translate(axisX, axisY, axisZ);
	...
	if (deg_order == "zyx") {
		...
		return (rotz * rotx * roty * trans);
	}
}

void EngineOSG::setupSphereOrientation() {
	viewer->getCamera()->setViewMatrix(CalcTransformation(axisX, distance, axisZ, azimuth, elevation, yaw, deg_order) * calcLookAt(viewer));
	...
	opm(1, 1) = opm(0, 0); //TODO - param (may deny face recognition)
...
void validateModelView(osg::ref_ptr<osgViewer::Viewer> viewer) {
	...
	for (unsigned int i = 0; i < QUADLET * QUADLET; i++) {
		if (fabs(OSGmvm[i] - OGLmvm[i]) > MAX_MVM_DIFF_ERR) {
			...
			sout << "Error: OGL modelview is different than OSG's. Either OGL modelview is the identity matrix or could not calculate the model transformations matrix." << endl;
		}
	}
}
...
public:
	...
	virtual void operator()(osg::RenderInfo& ri) const {
		...
		if (!foundInCache) {
			...
			gmdPair.meshData = engine.getMeshData();
			cache.insert(pair<string, NodeMeshDataPair> (filename, gmdPair));
			MyTexture tex;
		...
		if (!engine.writeFiles.empty())
		{
			osgDB::writeImageFile(*renderedImage, engine.writeFiles);
			const char* depthPath = engine.writeFiles.substr(engine.writeFiles.rfind('.')).append(".depth.pgm").c_str();
			WriteDepthFile(depthPath, imgDepth, width, height);
		}
		delete[] imgDepth;
...
class ContinuousImageCB: public osg::Camera::DrawCallback {

public:
	ContinuousImageCB(EngineOSG _engine, unsigned char* mem) : engine(_engine)
	{
		output = mem;
	}
	...
	virtual void operator()(osg::RenderInfo& ri) const 
	{
		...
		osg::ref_ptr<osg::Image> renderedImage = new osg::Image;
		renderedImage->readPixels(0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE);
		getRenderedImage(renderedImage->data(), width, height, output);
	}
...
void addModelToScene(osg::ref_ptr<osg::Node> model, osg::ref_ptr<osgViewer::Viewer> viewer)
{
	if(viewer->getSceneData() &&
		viewer->getSceneData()->getName().find("multiMode") != string::npos)
	{
		// Scene --> Higher --> Lower --> model

		osg::MatrixTransform* mtHigher = new osg::MatrixTransform(osg::Matrix::identity()); // any transformation
		osg::MatrixTransform* mtLower = new osg::MatrixTransform(osg::Matrix::identity());	// const rotation
		
		mtLower->addChild(model);
		mtHigher->addChild(mtLower);
		
		viewer->getSceneData()->asGroup()->addChild(mtHigher);// centerModel(mtHigher));
	}
	else
	{
...
osg::ref_ptr<osg::Geode> drawFromData(osg::ref_ptr<osgViewer::Viewer> viewer, MeshData &meshData) {
	...
	addModelToScene(geode.get(), viewer);
...
void drawFromFile(osg::ref_ptr<osgViewer::Viewer> viewer, string filename) {
	...
	addModelToScene(loadedModel.get(), viewer);
...
void EngineOSG::drawFromFileAndData() {
	...
	if (!USE_CACHE || !foundInCache) {
		...
		if (!loadedModel) {
			string str = "Error: file not found: ";
			str += filename;
			throw str.c_str();
		}
...
//					loadedModel->accept(sv);
		...
		if (filename.rfind(".wrl") != filename.npos) {
			...
			md->copy(gmd.meshData);
			loadedModel = drawFromData(viewer, *md);
		}
	}
	...
	if (USE_CACHE) {
		...
		if (!foundInCache) {
			...
			getmeshData().copy(gmdPair.meshData);
...
void EngineOSG::initData() {
	drawFromData(viewer, getmeshData());
}
...
bool EngineOSG::IsDone()
{
	return viewer->done();
}


class MyBoneVisitor : public osg::NodeVisitor {

	private: 
		//std::vector<osg::ref_ptr<osgAnimation::Bone>> _list;
		std::vector<osg::ref_ptr<osg::MatrixTransform>> _list;

	public:

		// set the traversal mode to TRAVERSE_ALL_CHILDREN
		MyBoneVisitor() : osg::NodeVisitor(TRAVERSE_ALL_CHILDREN)
		{
		}

		void apply(osg::Node &searchNode)
		{
			//osg::ref_ptr<osgAnimation::Bone> bn = dynamic_cast<osgAnimation::Bone*> (&searchNode);
			osg::ref_ptr<osg::MatrixTransform> bn = dynamic_cast<osg::MatrixTransform*> (&searchNode);
			if(bn)
			{
				_list.push_back(bn);
			}
		...
		//std::vector<osg::ref_ptr<osgAnimation::Bone>> GetList()
		std::vector<osg::ref_ptr<osg::MatrixTransform>> GetList()
		{
			return _list;
		}
};
...
void EngineOSG::startMultiMode()
{
	osg::Group* mainGroup = new osg::Group();
	mainGroup->setName("multiMode");
...
int EngineOSG::addModel(string filename, MeshData meshData, bool light, bool removeInterior, bool autoCenter, double axisX, double axisY, double axisZ, double azimuth, double elevation, double yaw)
{
	iActive = vecFilename.size();
	...
	osg::MatrixTransform* mtHigher = viewer->getSceneData()->asGroup()->getChild(iActive)->asTransform()->asMatrixTransform();
	osg::MatrixTransform* mtLower = mtHigher->getChild(0)->asTransform()->asMatrixTransform();

	mtLower->setMatrix(CalcTransformation(axisX, axisY, axisZ, azimuth, elevation, yaw, deg_order));

	osg::BoundingSphere bs = mtLower->getBound();
	osg::Vec3f center = bs.center();

	if(autoCenter)
	{
		mtLower->setMatrix(mtLower->getMatrix() * osg::Matrix::translate(-center));
	}

	mtHigher->setUserData(new osg::MatrixTransform(osg::Matrix::translate(center.x(), 0, center.z())));

	osg::StateSet* ss = mtHigher->getOrCreateStateSet();
	...
	MyBoneVisitor mbv;
	mbv.apply(*(viewer->getSceneData()->asGroup()->getChild(iActive)));
	//osgAnimation::BoneMap* bm = new osgAnimation::BoneMap();
	map<string, osg::MatrixTransform*>* bm = new map<string, osg::MatrixTransform*>();
	for(int i=0; i< mbv.GetList().size(); i++)
	{
		//osgAnimation::Bone* cur = mbv.GetList().at(i);
		osg::MatrixTransform* cur = mbv.GetList().at(i);
		//bm->insert(pair<string, osgAnimation::Bone*>(cur->getName(), cur));
		bm->insert(pair<string, osg::MatrixTransform*>(cur->getName(), cur));
		
		cout << cur->getName() << endl;
	}
	vecBoneMaps.push_back(bm);

	return iActive;
}

void EngineOSG::setModelTransformation(int index, double axisX, double axisY, double axisZ, double azimuth, double elevation, double yaw)
{
	iActive = index;
	
	osg::MatrixTransform* mtHigher = viewer->getSceneData()->asGroup()->getChild(iActive)->asTransform()->asMatrixTransform();
	
	osg::Matrix matSet(CalcTransformation(axisX, axisY, axisZ, azimuth, elevation, yaw, deg_order));

	mtHigher->setMatrix(matSet);
}

void EngineOSG::setModelTransformation(int index, double* A, double* R, double* T)
{
	iActive = index;
	...
	osg::Matrix mvm(mv);
	osg::Matrix mci(viewer->getCamera()->getInverseViewMatrix());

	osg::MatrixTransform* mtHigher = viewer->getSceneData()->asGroup()->getChild(iActive)->asTransform()->asMatrixTransform();
	
	osg::Matrix matSet(mvm*mci);
	
	mtHigher->setMatrix(matSet);
}

void EngineOSG::setModelVisibility(int index, bool show)
{
	iActive = index;
	if(show)
	{
		viewer->getSceneData()->asGroup()->getChild(iActive)->setNodeMask(0xffffffff);
	}
	else
	{
		viewer->getSceneData()->asGroup()->getChild(iActive)->setNodeMask(0x0);
	}
}

// osg3 cookbook: implementing a background image node
// http://forum.openscenegraph.org/viewtopic.php?t=2639 opencv to osg image
// http://stackoverflow.com/questions/19208024/aligning-osgviewer-camera-with-an-image work with rectangle instead of square to avoid resizing
void EngineOSG::SetBackgroundImage(unsigned char *rgbImage, int width, int height)
{
	// Load the background image
	if(!initBG)
	{
		memBG = new unsigned char[width*height*3];
	}

	memcpy(memBG, rgbImage, width*height*3);

	imgBG->setImage(width, height, 1, 
	...
	if(!initBG)
	{
		// map it to a quadrangle geometry
		osg::ref_ptr<osg::TextureRectangle> texture = new osg::TextureRectangle();
		texture->setImage( imgBG.get() );
		osg::ref_ptr<osg::Drawable> quad = osg::createTexturedQuadGeometry(osg::Vec3(), osg::Vec3(width, 0.0f, 0.0f), osg::Vec3(0.0f, height, 0.0f), 0, 0, width, height);
		quad->getOrCreateStateSet()->setTextureAttributeAndModes(0, texture.get());
		osg::ref_ptr<osg::Geode> geode = new osg::Geode;
		geode->addDrawable( quad.get() );

		// Prepare an HUD camera for the background image
		...
		// Prevent the background from being affected by the light, and set up the depth test values
		osg::StateSet* ss = camera->getOrCreateStateSet();
		ss->setMode( GL_LIGHTING, osg::StateAttribute::OFF );
		ss->setAttributeAndModes( new osg::Depth(osg::Depth::LEQUAL, 1.0, 1.0) );

		viewer->getSceneData()->asGroup()->addChild(camera.get());
...
map<string, Bone*>* EngineOSG::getModelBones(int index)
{
	iActive = index;

	map<string, Bone*>* ret = new map<string, Bone*>();
	//osgAnimation::BoneMap* bm = vecBoneMaps.at(index);
	map<string, osg::MatrixTransform*>* bm = vecBoneMaps.at(index);

	//for (osgAnimation::BoneMap::const_iterator iter = bm->begin(); iter != bm->end(); iter++)
	for (map<string, osg::MatrixTransform*>::const_iterator iter = bm->begin(); iter != bm->end(); iter++)
	{
		//osgAnimation::Bone* cur = iter->second;
		osg::MatrixTransform* cur = iter->second;
		Bone* b = new Bone(iter->first, 0,0,0);
		ret->insert(pair<string, Bone*>(b->name, b));

		
		osgAnimation::Bone* curBone = dynamic_cast<osgAnimation::Bone*> (cur);
		if(curBone)
		{
			// remove update callback
			// it returns the bone to the original location after each frame
			// if left as-is, there is no need to track current bone position
			// however, looks like it interups some models bones movement
			curBone->setUpdateCallback(NULL);
		}
	}
...
void EngineOSG::moveModelBones(int index, vector<Bone*>* vec)
{
	//osgAnimation::BoneMap* bm = vecBoneMaps.at(index);
	map<string, osg::MatrixTransform*>* bm = vecBoneMaps.at(index);
	...
	{
		Bone* animInfo = vec->at(i);
		if(bm->find(animInfo->name) !=bm->end())
		{
			//osgAnimation::Bone* cur = bm->at(animInfo->name);
			osg::MatrixTransform* cur = bm->at(animInfo->name);
			if(cur)
			{
				osgAnimation::Bone* b = dynamic_cast<osgAnimation::Bone*> (cur);
				if(b)
				{
					b->setMatrixInSkeletonSpace(b->getMatrixInSkeletonSpace() * osg::Matrix::translate(osg::Vec3f(animInfo->posX, animInfo->posY, animInfo->posZ)));
				}
				else
				{
					cur->setMatrix(cur->getMatrix() * osg::Matrix::translate(osg::Vec3f(animInfo->posX, animInfo->posY, animInfo->posZ)));
				}
			}
		}
		else
		{
			cout << "Err: Bone not found - " << animInfo->name << endl;
		}
	}
}

void EngineOSG::RegisterForImages(unsigned char* mem)
{
	viewer->getCamera()->setFinalDrawCallback(new ContinuousImageCB(*this, mem));
}
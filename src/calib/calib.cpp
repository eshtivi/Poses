...
void Calib::GetEuler(double* Rin, double* out)
{
	double rotMatrs[3 * 3];

	transpose(Rin, 3, 3, rotMatrs);

	CvMat rotation_matrices = cvMat(9, 1, CV_64FC1, rotMatrs);
...
void Calib::run(int width, int height, int numPoints, double *imgPointsIn, double *objPointsIn,
...
//		memset(cameraMatrix, 0, 9 * sizeof(cameraMatrix[0]));
	...
	try {
		...
		if (AOutput != NULL && ROutput != NULL && TOutput != NULL) {
			...
			/**AOutput = eulerAngles.x;
			*ROutput = eulerAngles.y;
			*TOutput = eulerAngles.z;*/
		}
...
int main() {
	...
	try {
		Calib *c = new Calib();
		c->run(width, height, numPoints, imgPoints, objPoints, NULL, NULL, NULL, Ain, 0, usePOSIT,
